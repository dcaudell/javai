 ...
 * Tag by id itself; {@link JavAITagging} resolves {@code tagId} through the caller-supplied
 * {@code JavAIRepository<Tag>} instead (see that class's own javadoc for why). Exists specifically for
 * {@link JavAITagging#classify}'s diff against existing {@code source = "auto"} associations, which needs
