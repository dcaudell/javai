 ...
 *       shape in javai-runtime): {@code registerSyncListener}/{@code registerAsyncListener} plus the
 ...
 *       selection, not the type-level selection {@code javai-agent}'s {@code JavAIWeaver} uses) and
 ...
 * </ul>
 *
 * <p><b>Why this module has its own weaver instead of extending {@code javai-agent}'s:</b> {@code
 * javai-agent} depends only on {@code javai-annotations} + {@code javai-runtime} and is deliberately the
 * first thing proven in Phase 0 ("prove the weaving mechanism itself... before building out javai-runtime
 * and javai-collections in full" -- CLAUDE.md). If this module's weaving lived in {@code javai-agent}
 * instead, and an Agentic Listener implementation eventually wants to depend on {@code javai-completion}
 * (to actually call an LLM) or {@code javai-collections}/{@code javai-runtime} (to ground its decision in
 * the object graph), {@code javai-agent} would end up needing those too -- pulling the one module meant to
 * be provable earliest and cheapest all the way downstream of the rest of the reactor. Keeping Agentic
 * Supervision's weaver independent (depending only on {@code javai-annotations}, same tier as {@code
 * javai-agent}) means it's a second, parallel, equally-early risk spike instead of a blocker on the first
 * one. Nothing here needs to know about vectorization at all -- see {@link
