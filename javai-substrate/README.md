...
Depends on `javai-annotations` + `javai-vector` + `javai-model` (it weaves calls into runtime hooks —
`javai-vector` for `EmbeddingVector`/dirty-tracking primitives, `javai-model` for the `JavAIVectorizable`
contract and `JavAIRuntime` engine it wires; see `javai-model`'s own package-info.java for why those two
types live there instead of `javai-vector`). Everything below the line an application programmer is
expected to think about: the mechanism that makes the other five extension areas real, without being part
of their public surface. Nothing here changes the *behavior* described in the other `doc/spec/*.md` files —
it only changes how fast, or by what mechanism, that behavior is delivered.
...
|---|---|---|---|
| `javai-substrate` / weaver | Build plugin (Maven/Gradle) or `-javaagent` | Build- or load-time bytecode enhancer (ByteBuddy-based) — the Hibernate-enhancement pattern applied to vectorization | **Phase 0** |
...
- `JavAIWeaver` — installs a load-time ByteBuddy `AgentBuilder` transformer matching
  ...
  `javai-model`'s `JavAIRuntime` statics. No algorithmic logic lives here; see `JavAIRuntime`'s javadoc
  for why.
