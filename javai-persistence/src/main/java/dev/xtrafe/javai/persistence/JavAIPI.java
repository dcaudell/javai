 ...
 * {@link Proxy}, backed by whichever {@link JavAIPersistenceConfig.Backend} is configured.
 *
 * <pre>{@code
 ...
 *
 * <p><b>Registration-before-use</b>: call {@link #repository(Class)} for every repository interface an
 * application needs *before* invoking methods on any of them. The Postgres backend's internal
 * {@code SessionFactory} accumulates entity classes across these calls and is built, once, lazily, on
 * first actual method invocation -- Hibernate's metadata is immutable once built, so an entity type
 * registered only after that point would never be picked up.
 */
public final class JavAIPI {
    ...
    /** Overrides the self-contained, system-property-derived default. See {@link JavAIPersistenceConfig}. */
    public static void configurePersistence(JavAIPersistenceConfig config) {
        JavAIPI.config = config;
    }

    @SuppressWarnings("unchecked")
    public static <R extends JavAIRepository<?>> R repository(Class<R> repositoryInterface) {
        ...
        RepositoryBackend backend = backendFor(currentConfig());
    ...
    private static JavAIPersistenceConfig currentConfig() {
        ...
        if (current == null) {
            synchronized (JavAIPI.class) {
                current = config;
                if (current == null) {
                    current = JavAIPersistenceConfig.fromSystemProperties();
                    config = current;
                }
            }
        }
