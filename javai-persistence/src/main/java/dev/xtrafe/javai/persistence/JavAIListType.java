 ...
 * {@link PersistentJavAIList} instead of its own {@code PersistentBag}. In the real implementation this
 * would be attached automatically by the backend's generated mapping, so a consumer never names it.
 */
public class JavAIListType implements UserCollectionType {
    ...
    public PersistentCollection<?> instantiate(
        ...
        return new PersistentJavAIList<>(session);
    }

    @Override
    @SuppressWarnings({"unchecked", "rawtypes"})
    public PersistentCollection<?> wrap(SharedSessionContractImplementor session, Object collection) {
        return new PersistentJavAIList<>(session, (Collection) collection);
    }
