 ...
 * String on demand ({@link #toString()}/{@link #toContext(PromptContext)}). Lives in {@code javai-runtime},
 ...
 * {@code package-info.java}: {@code javai-completion} depends on {@code javai-runtime}, not the reverse, so
 ...
 * </ul>
 *
 * <p><b>{@link #defaultMarshall(Object)} uses GSON, not Jackson</b> -- deliberately: {@code javai-runtime}
 * takes no Jackson dependency of its own (only {@code javai-completion} sees Jackson at all, transitively
 * via Spring AI). GSON's default reflective serialization has no cycle guard equivalent to this project's
 ...
 * {@code @JavAIVectorizable} class carries internal bookkeeping fields (a cached {@link EmbeddingVector},
